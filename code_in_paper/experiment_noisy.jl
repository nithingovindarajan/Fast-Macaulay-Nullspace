...
noises = 10.0.^(-5:.1:1)
nMethods = 7
results = zeros(nMethods, length(noises), reps)
for (i, η) in enumerate(noises)
    for rep = 1:reps
        println("Noise ", η,", rep ", rep)
        Random.seed!(rep+1)
        p1 = randnBivariatePolynomial(d_Σ)
        p2 = randnBivariatePolynomial(d_Σ)
        p3 = randn()*p1+randn()*p2
        p = [p1, p2, p3]
        for j = 1:3
            p[j] = add_noise(p[j], η)
        end
    ...
    # file = matopen("stability_noisy_m5_p1_1000.mat", "w")
    # write(file, "results", results)
    # close(file)
    display(mean(results[[2, 4, 5, 7],:,:]; dims=3))
    display(median(results[[2, 4, 5, 7],:,:]; dims=3))
    display(std(results[[2, 4, 5, 7],:,:]; dims=3))
end