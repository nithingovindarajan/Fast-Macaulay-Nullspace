...
function rel_ind_err(A, H)
    A = Matrix(A)
    Q = Matrix(qr(Matrix(H), ColumnNorm()).Q)
    return(opnorm(A * Q) / (opnorm(A)))
end
function rel_ind_err_old(A, H)
    A = Matrix(A)
    H = Matrix(H)
...
function add_shift_noise(p::BivariatePolynomial{T}, eta::S) where {T<:Number,S<:Number}
    r = deepcopy(p.coeff)
    r[1][1:1] .+= eta * randn(T)
    return BivariatePolynomial{T}(r)
end
...
function schur_algorithm(A::CauchyLike{TC}; Kmax::Int64=minimum(size(A), init=0), qr_initialization=qr_initial, ABSTOL::TF=1E-12, CP::Bool = false) where {TF <: Real, TC <: Number}
    ...
    # Prealloc
    a = Vector{eltype(U2)}(undef, size(U2, 1))
    w = Vector{eltype(U2)}(undef, size(U2, 1))
    g = Vector{eltype(V2)}(undef, size(V2, 1))
    uvt = Vector{eltype(V2)}(undef, size(V2, 2))

    if CP
        UV = Matrix{eltype(U2)}(undef, size(U2,1), size(V2,1))
        UVa = Matrix{Float64}(undef, size(U2,1), size(V2,1))
    else
        U = copy(A.U)
        V = copy(A.V)
    ...
    for k = 1:Kmax
        if CP
            @views mul!(UV[k:end, k:end], U2[k:end, :], V2[k:end, :]')
            @views UVa[k:end, k:end] .= abs.(UV[k:end, k:end]./(μ[k:end] .- transpose(ν[k:end])))
            @views max2nrm, ind = findmax(UVa[k:end, k:end])
            i = ind[1]
            j = ind[2]
...
function null(A::CauchyLike{TC}; ABSTOL::TF=1E-12, Kmax::Integer=minimum(size(A)), qr_initialization=qr_initial_macaulay, CP::Bool = false) where {TF <: Real, TC <: Number}
    _, Π_2, N = schur_algorithm(A; ABSTOL=ABSTOL, qr_initialization=qr_initialization, Kmax, CP=CP)
...
function null(A::MacaulayBivariate{TC}; ABSTOL::TF=1E-12, Kmax::Integer=minimum(size(A)), gen_φ = generate_φ_greedy, CP::Bool = false) where {TF <: Real, TC <: Number}
    ...
    Hhat = null(Ahat; ABSTOL=ABSTOL, qr_initialization=qr_initial_macaulay, Kmax, CP)
